...
class GoogleSheetsSimpleService:
    ...
    def setup_google_sheets(self):
        """
        Configura la conexión con Google Sheets usando API key
        """
        try:
            ...
            if not sheet_id:
                logger.warning("GOOGLE_SHEETS_ID no configurado, usando datos de ejemplo")
                self.create_sample_data()
                return
            
            if not api_key:
                logger.warning("GOOGLE_API_KEY no configurado, usando datos de ejemplo")
                self.create_sample_data()
                return
            ...
            logger.info("Configuración de Google Sheets API establecida")
    ...
    def load_sheets_data(self) -> bool:
        """
        Carga los datos desde Google Sheets usando la API pública
        """
        try:
            if not hasattr(self, 'sheet_id') or not hasattr(self, 'api_key'):
                logger.warning("Google Sheets no configurado, usando datos de ejemplo")
                self.create_sample_data()
                return True
            
            # URL para obtener datos de Google Sheets como CSV
            # Nota: La hoja debe ser pública o compartida con "Anyone with the link can view"
            sheet_name = "PRECIOS FOB"  # Nombre de la hoja
            url = f"https://sheets.googleapis.com/v4/spreadsheets/{self.sheet_id}/values/{sheet_name}?key={self.api_key}"
            
            logger.info(f"Intentando cargar datos desde: {url}")
            
            response = requests.get(url)
            
            if response.status_code != 200:
                logger.error(f"Error accediendo a Google Sheets: {response.status_code} - {response.text}")
                self.create_sample_data()
                return False
            ...
            if 'values' not in data:
                logger.error("No se encontraron datos en la respuesta de Google Sheets")
                self.create_sample_data()
                return False
            ...
            logger.info(f"Datos cargados desde Google Sheets API: {total_sizes} tallas en {len(self.prices_data)} productos")
            
            return True
            
        except Exception as e:
            logger.error(f"Error cargando datos de Google Sheets API: {str(e)}")
            self.create_sample_data()
            return False
